...
public class AllianceBehavior : SingletonObject<AllianceBehavior>
{
    /*private void Start()
    {
        EventManager.DailyTickClanEvent.AddNonSerializedListener(this, DailyTickClan);
        EventManager.WarDeclared.AddNonSerializedListener(this, WarDeclared);
        EventManager.AllianceFormed.AddNonSerializedListener(this, AllianceFormed);
    }
    ...
    private static void ConsiderFormingAlliances(Kingdom kingdom)
    {
        ...
        foreach (var potentialAlly in potentialAllies)
            if (RandomExtention.NextFloat < 0.05f && AllianceScoringModel.Instance.ShouldFormBidirectional(kingdom, potentialAlly))
    ...
    private static void ConsiderBreakingAlliances(Kingdom kingdom)
    {
        ...
        {
            if (RandomExtention.NextFloat < 0.05f
